...
PLOT_FILEPATH_TEMPLATE = 'Likelihood/Round2/Plots/count_dist-p_mle-leq-{}-{}.png'
CSV_FILEPATH_TEMPLATE = 'Likelihood/Round2/Distributions/{}-p_mle-leq-{}-{}.csv'
LOG_TEMPLATE = 'Likelihood/Round2/Logs/energy-and-p_over_time-{}-{}.1.txt'
LOG_BESTS_TEMPLATE = 'Likelihood/Round2/Logs/best_over_time-{}-{}.1.txt'
...
def print_setup(bit_P, score_P, old_score_P=None):
    print('{Pr(bit i = 1): i \in', indices, '} = ', bit_P[indices].tolist())
    print('-log(Pr(Matches >= 13)) = {}'.format(score_P))
...
def run_annealing(model, start_year, stop_year):
    ...
    for year in np.arange(start=start_year, stop=stop_year + 1, step=1):
        ...
        # P[0] = 1.0
        # P[1] = 0.5928450489751618
        # P[2] = 0.6950938557087003
        # P[3] = 0.9189491423484074
        # P[4] = 0.7747201971300952
        # P[5] = 1.0
            ...
            for trial_at_fixed_temperature in range(10000):
                ...
                if np.random.rand() < accept_p:
                    ...
                    if new_pr < best_pr:
                        ...
                        best_log.write('{},{}: {}\n'.format(counter, new_pr, ','.join(new_P[indices].astype(str))))
