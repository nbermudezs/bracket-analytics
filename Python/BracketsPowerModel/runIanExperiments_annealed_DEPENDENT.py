...
def performExperiments(numTrials, year, batchNumber, model):
    ...
    for n in range(numTrials):
        ...
        for scoring_year in range(2013, 2019):
            correctVector = getActualBracketVector(scoring_year)
            newBracketScore = scoreBracket(newBracketVector, correctVector)
            brackets[scoring_year].append({'bracketVector': newBracketString, 'score': newBracketScore})

    for scoring_year in range(2013, 2019):
        correctVector = getActualBracketVector(scoring_year)
        bracketListDict = {'year': scoring_year, 'actualBracket': ''.join(str(bit) for bit in correctVector), 'brackets': brackets[scoring_year]}
        if numTrials < 1000:
        ...
        outputFilename = '{2}/generatedBrackets_{0}_{1}.json'.format(model['modelName'], scoring_year, batchFolderName)
...
for modelDict in modelsList:
    ...
    for batchNumber in range(numBatches):
        ...
        performExperiments(numTrials, modelDict.get('generator_year'), batchNumber, modelDict)
