...
all_alphas = {}
def computeAllAlpha(model):
    for year in range(2013, 2019):
        prior_brackets = [b for y, b in all_brackets.items() if y < year]
        cuts = split_array(prior_brackets, model['ensemble']['window'])
        all_alphas[year] = [compute_all_alphas(cut) for cut in cuts]
...
def getAlpha(s1, s2, model, year, roundNum):
    alphas = all_alphas[year]
    n = len(alphas)
    i = np.random.randint(0, n)
    if roundNum > 1:
        return alphas[i][roundNum]
    if s1 in alphas[i][roundNum]:
        return alphas[i][roundNum][s1].get(s2, 0.)
    else:
        return 0.
